...
contract Faucet is EIP712MetaTransaction("EthFaucet", "1") {
    ...
    modifier hasEnoughBalance() {
        require(address(this).balance >= 0.05 ether, "Not enough Eth");
        _;
    }

    modifier hasNotUsedFaucet() {
        require(
            block.timestamp - 24 hours >= alreadyUsed[msgSender()],
            "Just once a day"
        );
        _;
    }
    ...
    function sendEther() public hasEnoughBalance hasNotUsedFaucet isNotGreedy {
        ...
        payable(msgSender()).transfer(0.05 ether);
        emit Send(msgSender(), 0.05 ether);
    }
